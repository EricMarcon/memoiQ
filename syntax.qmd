...
At the moment, it is restricted to simple text without format, math or cross references (@tbl-kable), in contrast with figure captions (see @fig-pressure).
...
To use the full width of the page, like @tbl-Paracou, `longtable` is set to `FALSE` and `table.envir = "table"` is added in the aguments of `kable()`.
