...
Figures can be created by the R code (@fig-pressure).
With Bookdown, a label is associated with each figure: its name is `fig:xxx` where `xxx` is the name of the R code snippet.
References are made with the command `@fig-xxx`.

The header of the code snippet of @fig-pressure is:
...
@fig-pressure2 benefits from an improved caption.

```{r}
#| label: fig-pressure2
#| echo: false
#| fig.cap: "Title with _italic_, math ($\\sqrt\\pi$) and reference to @fig-pressure"
plot(pressure)
```
...
They can be numbered: see @eq-disk, using the `\equation` environment.
...
In all cases, the call to the reference is made by the command `@sec-yyy`.
...
For PDF documents, the bibliography is handled by BibLaTeX, see section @sec-index.
...
:::{#pythbox .greybox data-latex='[frametitle=Pythagorean theorem]' title='Pythagorean theorem'}
